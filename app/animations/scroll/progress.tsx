...
export default function ScrollProgressScreen() {
      ...
      >
        <View className="border-b border-slate-200 px-6 py-4">
          <Text className="mt-1 text-sm text-slate-600">
            Show read progress as a bar. Interpolate scroll position to bar width.
            Useful for articles, profiles, or onboarding.
          </Text>
          <View className="mt-3 rounded-lg bg-slate-100 p-3">
            <Text className="font-mono text-xs text-slate-700">
              width = interpolate(scrollY, [0, max], [0, barWidth])
            </Text>
            <Text className="mt-2 text-xs text-slate-600">
              Scroll to fill the progress bar
            </Text>
          </View>
        </View>
        <ScrollProgress />
