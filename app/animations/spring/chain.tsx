import { Chain } from "@/components/spring/Chain";
import { Ionicons } from "@expo/vector-icons";
import { useRouter } from "expo-router";
import React from "react";
import { ScrollView, Text, TouchableOpacity, View } from "react-native";

export default function SpringChainScreen() {
      ...
      >
        <View className="border-b border-slate-200 px-6 py-4">
          <Text className="mt-1 text-sm text-slate-600">
            Chain springs with withSequence or run multiple springs in parallel.
            Each block bounces in sequence.
          </Text>
          <View className="mt-3 rounded-lg bg-slate-100 p-3">
            <Text className="font-mono text-xs text-slate-700">
              withSequence(withSpring(...), withSpring(...))
            </Text>
